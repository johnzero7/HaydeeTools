...
class addon_updater_install_manually(bpy.types.Operator):
	...
	def draw(self, context):
		...
		if False:
			layout.label(text="There was an issue trying to auto-install")
		else:
			col = layout.column()
			col.scale_y = 0.7
	...
	def draw(self, context):
		...
		if updater.auto_reload_post_update == False:
			...
			if "just_restored" in saved and saved["just_restored"] == True:
				col = layout.column()
				col.scale_y = 0.7
				col.label(text="Addon restored")
				col.label(text="Restart blender to reload.")
...
def register(bl_info):
	...
	updater.repo = "HaydeeTools"

	#updater.addon = # define at top of module, MUST be done first

	# Website for manual addon download, optional but reocmmended to set
	updater.website = "http://johnzero7.github.io/HaydeeTools/"
